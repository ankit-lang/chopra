...
export default function TandooriPage({ params }: Props) {
  ...
  return (
    <>
      ...
      <section className="bg-white py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="prose prose-lg max-w-none text-[#1A1A1A] space-y-5">
            {isNl ? (
              <>
                <p>De meeste kip die als tandoori wordt verkocht in Den Haag, wordt bereid in een gasoven. Gemarineerd, soms. Gekleurd met rood voedselkleurstof voor de uitstraling. Maar gebakken in een doos hete lucht op 250 graden Celsius. De verkoolde rand, de rokerige korst, de geur die de tafel bereikt voordat het bord aankomt - die combinatie vereist een temperatuur die geen enkele gewone oven kan bereiken. Dit is de reden waarom <Link href={`${base}/halal-food-den-haag`} className="text-[#06068a] hover:underline">authentiek halal tandoori Den Haag</Link> zo zeldzaam goed is als u het eenmaal proeft.</p>
                <p>De kleioven bij Chopras Indian Restaurant op Leyweg 986 bereikt 400 graden Celsius. Op die temperatuur verloopt de Maillard-reactie in seconden, niet in minuten. De yoghurtmarinade op het kipoppervlak verkoolt bijna onmiddellijk en vormt een gekarameliseerde korst die het vocht insluit, terwijl de intensieve stralingswarmte van de kleienwanden het vlees vanuit alle richtingen tegelijkertijd gaart. Er bestaat geen manier om dit te repliceren bij lagere temperaturen. Er is geen alternatieve route.</p>
                <p>Het klei zelf speelt ook een rol. In tegenstelling tot metaal is klei een slechte warmtegeleider - het houdt warmte vast en straalt die zachtjes en gelijkmatig uit, zoals alleen een materiaal kan dat tweeduizend jaar voor dit doel is gebruikt. Chopras stookt de tandoor uren voor de eerste gast aankomt. Elke chicken tikka en seekh kebab die die avond de keuken verlaat, is bereid op de temperatuur waarvoor het gerecht is bedacht. Lees waarom Chopras wordt erkend als het <Link href={`${base}/blog/best-indian-restaurant-den-haag`} className="text-[#06068a] hover:underline">beste Indiaas restaurant in Den Haag</Link> op basis van 1100+ Google-recensies.</p>
              </>
            ) : (
              <>
                <p>Most chicken sold as tandoori in Den Haag is cooked in a gas oven. Marinated, sometimes. Coloured with red food dye for appearances. But cooked in a box of hot air at 250 degrees Celsius. The char that defines real tandoori - the scorched edge, the smoky crust, the smell that reaches the table before the plate does - requires a temperature that no conventional oven can reach. This is why <Link href={`${base}/halal-food-den-haag`} className="text-[#06068a] hover:underline">authentic halal tandoori Den Haag</Link> is so rare and so immediately recognisable when you find it.</p>
                <p>The clay oven at Chopras Indian Restaurant on Leyweg 986 reaches 400 degrees Celsius. At that temperature, the Maillard reaction occurs in seconds, not minutes. The yogurt marinade on the chicken surface scorches almost immediately, creating a caramelised crust that seals in moisture while the intense radiant heat from the clay walls cooks the meat from every direction simultaneously. There is no way to replicate this at lower temperatures. No workaround exists.</p>
                <p>The clay itself matters. Unlike metal, clay is a poor conductor of heat - it holds and radiates warmth gently and evenly, the way only a material used for this purpose for two thousand years can. Chopras fires the tandoor hours before the first guest arrives. Every chicken tikka Den Haag and seekh kebab that leaves the kitchen that night has been cooked at the temperature it was designed for. Read why Chopras is recognised as the <Link href={`${base}/blog/best-indian-restaurant-den-haag`} className="text-[#06068a] hover:underline">best Indian restaurant in Den Haag</Link> across 1100+ Google reviews.</p>
              </>
            )}
          </div>
        </div>
      ...
      <section className="bg-white py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="font-heading text-3xl md:text-4xl text-[#06068a] mb-6 text-center">
          ...
          <div className="prose prose-lg max-w-none text-[#1A1A1A] space-y-4 mb-8">
            {isNl ? (
              <p>Elk tandoori gerecht bij Chopras is <Link href={`${base}/halal-food-den-haag`} className="text-[#06068a] hover:underline">volledig halal gecertificeerd</Link> en vers bereid bij elke bestelling. De kip wordt de avond van tevoren gemarineerd. Het lam voor de seekh kebab wordt elke ochtend vers gemalen met verse specerijen. Paneer tikka wordt dagelijks vers bereid. Er zijn geen bevroren componenten in de tandoor-sectie van het Chopras-menu.</p>
            ) : (
              <p>Every tandoori dish at Chopras is <Link href={`${base}/halal-food-den-haag`} className="text-[#06068a] hover:underline">fully halal certified</Link> and freshly prepared to order. The chicken is marinated the night before. The lamb for seekh kebab is ground fresh each morning with fresh spices. Paneer tikka is made fresh daily. There are no frozen components in the tandoor section of the Chopras menu.</p>
            )}
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            {[
              ...
              <div key={item.name} className="bg-[#F7F8FC] rounded-xl p-5 border-l-4 border-white">
                <h3 className="font-heading text-lg text-[#06068a] mb-1">
                  <Link href={`${base}${item.href}`} className="text-[#06068a] hover:underline">{item.name}</Link> - {item.price}
                </h3>
      ...
      <section className="bg-white py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="font-heading text-3xl md:text-4xl text-[#06068a] mb-8">
          ...
          <div className="prose prose-lg max-w-none text-[#1A1A1A] space-y-5">
            {isNl ? (
              <>
                ...
                <p>Chicken tikka bij Chopras wordt een nacht gemarineerd in volle yoghurt met verse gember, knoflook, rode chili, kurkuma en een masalamelange die in eigen huis wordt gemalen van hele specerijen die rechtstreeks uit India worden geïmporteerd. De yoghurt is niet decoratief. Melkzuur in yoghurt denatureert de eiwitten op het kipoppervlak en opent de vezels, zodat de specerijen diep in het vlees kunnen doordringen in plaats van alleen op het oppervlak te blijven. Bekijk het <Link href={`${base}/menu`} className="text-[#06068a] hover:underline">volledige Noord-Indiase menu bij Chopras</Link> om te zien hoe de tandoori gerechten naast de curryselectie staan.</p>
                <p>Wanneer de gemarineerde kip de 400-graden tandoor binnengaat, is het kruidenprofiel al aanwezig door het hele stuk vlees. Het vuur werkt dan op de buitenkant. Het resultaat is kip die van buiten rokerig en verkoold is, door en door gekruid, en van binnen sappig - drie kwaliteiten die moeilijk afzonderlijk te bereiken zijn en bijna onmogelijk samen zonder zowel de juiste voorbereiding als de juiste temperatuur. Chopras serveert dit elke avond van dinsdag tot en met zondag vanaf 16:30, naast <Link href={`${base}/biryani-den-haag`} className="text-[#06068a] hover:underline">biryani Den Haag</Link> en het volledige curry-menu.</p>
              </>
            ) : (
              <>
      ...
      <section className="btn-gradient  py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-white/90 text-lg leading-relaxed">
            {isNl ? (
              <p className='text-white/80'>Authentieke tandoori Den Haag vindt u bij Chopras Indian Restaurant, Leyweg 986, 2545 GW Den Haag, open van dinsdag tot en met zondag van 16:30 tot 22:30. De kleioven bereikt 400 graden Celsius - de enige temperatuur waarbij chicken tikka en seekh kebab hun karakteristieke rokerige korst ontwikkelen. Beoordeeld met 4,9 sterren op basis van 1100+ Google-recensies. Alle tandoori gerechten zijn <Link href={`${base}/halal-food-den-haag`} className="text-white hover:underline font-semibold">volledig halal gecertificeerd</Link> en een nacht van tevoren gemarineerd. <Link href={`${base}/contact`} className="text-white hover:underline font-semibold">Reserveer een tafel</Link> of bestel online.</p>
            ) : (
              <p className='text-white/80'>Authentic tandoori Den Haag is at Chopras Indian Restaurant, Leyweg 986, 2545 GW Den Haag, open Tuesday to Sunday from 16:30 to 22:30. The clay oven reaches 400 degrees Celsius - the only temperature at which chicken tikka and seekh kebab develop their characteristic smoky crust. Rated 4.9 stars from 1100+ Google reviews. All tandoori dishes are <Link href={`${base}/halal-food-den-haag`} className="text-white hover:underline font-semibold">fully halal certified</Link> and marinated overnight. <Link href={`${base}/contact`} className="text-white hover:underline font-semibold">Reserve a table</Link> or order online.</p>
            )}
          </div>
        </div>
      </section>
      ...
      <section className="bg-white py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="font-heading text-3xl md:text-4xl text-[#06068a] mb-8 text-center">
            {isNl
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Link href={`${base}/biryani-den-haag`} className="block p-6 bg-[#F7F8FC] rounded-lg border border-gray-200 hover:border-[#06068a]/30 hover:shadow-lg transition-all">
              <p className="text-[#06068a] text-sm font-semibold uppercase tracking-widest mb-2">Biryani</p>
              <p className="text-[#06068a] font-semibold">{isNl ? 'Authentieke biryani Den Haag' : 'Authentic biryani Den Haag'}</p>
            </Link>
            <Link href={`${base}/butter-chicken-den-haag`} className="block p-6 bg-[#F7F8FC] rounded-lg border border-gray-200 hover:border-[#06068a]/30 hover:shadow-lg transition-all">
              <p className="text-[#06068a] text-sm font-semibold uppercase tracking-widest mb-2">{isNl ? 'Kip Curry' : 'Chicken Curry'}</p>
            ...
            <Link href={`${base}/naan-den-haag`} className="block p-6 bg-[#F7F8FC] rounded-lg border border-gray-200 hover:border-[#06068a]/30 hover:shadow-lg transition-all">
              <p className="text-[#06068a] text-sm font-semibold uppercase tracking-widest mb-2">Naan</p>
              <p className="text-[#06068a] font-semibold">{isNl ? 'Tandoori naan Den Haag' : 'Tandoori naan Den Haag'}</p>
            </Link>
            <Link href={`${base}/indian-food-delivery-den-haag`} className="block p-6 bg-[#F7F8FC] rounded-lg border border-gray-200 hover:border-[#06068a]/30 hover:shadow-lg transition-all">
              <p className="text-[#06068a] text-sm font-semibold uppercase tracking-widest mb-2">{isNl ? 'Bezorging' : 'Delivery'}</p>
              <p className="text-[#06068a] font-semibold">{isNl ? 'Indiaas eten bezorgen Den Haag' : 'Indian food delivery Den Haag'}</p>
            </Link>
            <Link href={`${base}/mutton-rogan-josh-den-haag`} className="block p-6 bg-[#F7F8FC] rounded-lg border border-gray-200 hover:border-[#06068a]/30 hover:shadow-lg transition-all">
              <p className="text-[#06068a] text-sm font-semibold uppercase tracking-widest mb-2">{isNl ? 'Lam Curry' : 'Lamb Curry'}</p>
            ...
            <Link href={`${base}/catering`} className="block p-6 bg-[#F7F8FC] rounded-lg border border-gray-200 hover:border-[#06068a]/30 hover:shadow-lg transition-all">
              <p className="text-[#06068a] text-sm font-semibold uppercase tracking-widest mb-2">Catering</p>
              <p className="text-[#06068a] font-semibold">{isNl ? 'Indiaas catering Den Haag voor uw evenement' : 'Indian catering Den Haag for your event'}</p>
            </Link>
          </div>
          <div className="mt-8 text-center space-y-4">
            ...
            <p className="text-[#1A1A1A] text-base">
              {isNl ? (
                <>Bekijk het <Link href={`${base}/menu`} className="text-[#06068a] hover:text-[#0000B3] font-semibold">volledige menu</Link> of <Link href={`${base}/contact`} className="text-[#06068a] hover:text-[#0000B3] font-semibold">maak een reservering</Link>.</>
              ) : (
                <>View the <Link href={`${base}/menu`} className="text-[#06068a] hover:text-[#0000B3] font-semibold">full menu</Link> or <Link href={`${base}/contact`} className="text-[#06068a] hover:text-[#0000B3] font-semibold">book a table at Chopras Indian Restaurant Den Haag</Link>.</>
              )}
            </p>
          </div>
        </div>
