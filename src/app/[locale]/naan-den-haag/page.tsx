          ...
          <div className="font-body text-[#1A1A1A]/70 text-lg leading-relaxed space-y-5">
            {isNl ? (
              <>
                ...
                <p><strong>Knoflooknaan.</strong> De meest bestelde. Vers deeg, hete tandoor, dan direct een kwast gesmolten boter en fijngehakte knoflook zodra het eruit komt. De knoflook gaart licht tegen het warme oppervlak. Het zit niet rauw bovenop. Bestellen naast <Link href={`${base}/butter-chicken-den-haag`} className="text-[#06068a] hover:text-[#0000B3] font-semibold">butter chicken Den Haag</Link> is bijna een verplichting.</p>
                <p><strong>Boternaan.</strong> De klassieke. Gewone tandoori naan met een royale kwast geklaarde boter. Eet het met <Link href={`${base}/dal-makhani-den-haag`} className="text-[#06068a] hover:text-[#0000B3] font-semibold">dal makhani Den Haag</Link> en er blijft niets op het bord over.</p>
                ...
                <p><strong>Keema naan.</strong> Gekruid gehakt lamsvlees gevouwen in het deeg. Halal gecertificeerd. Deze is op zichzelf al een vol bord, maar de meeste gasten bestellen hem toch naast een curry van het <Link href={`${base}/menu`} className="text-[#06068a] hover:text-[#0000B3] font-semibold">volledige Chopras menu</Link>.</p>
                <p><strong>Gewone naan.</strong> Het origineel. Tandoor hitte, deeg, verder niets. Als de basis goed is, is er niets anders nodig. Bij 400 graden verdient de gewone naan zijn plek op elk bord. Combineer het met <Link href={`${base}/mutton-rogan-josh-den-haag`} className="text-[#06068a] hover:text-[#0000B3] font-semibold">mutton rogan josh Den Haag</Link> voor een samenspel dat voor zichzelf spreekt.</p>
              </>
            ) : (
              <>
                ...
                <p><strong>Garlic naan.</strong> The most ordered. Fresh dough, hot tandoor, then immediately a brush of melted butter and finely chopped garlic the moment it comes out. The garlic cooks slightly against the warm surface. It does not sit raw on top. Ordering this alongside <Link href={`${base}/butter-chicken-den-haag`} className="text-[#06068a] hover:text-[#0000B3] font-semibold">butter chicken Den Haag</Link> is practically obligatory.</p>
                <p><strong>Butter naan.</strong> The classic. Plain tandoor naan with a generous brush of clarified butter. Eat it with <Link href={`${base}/dal-makhani-den-haag`} className="text-[#06068a] hover:text-[#0000B3] font-semibold">dal makhani Den Haag</Link> and nothing is left on the plate.</p>
                ...
                <p><strong>Keema naan.</strong> Spiced minced lamb folded into the dough. Halal certified. This one is a full plate on its own, though most guests still order it alongside a curry from the <Link href={`${base}/menu`} className="text-[#06068a] hover:text-[#0000B3] font-semibold">full Chopras menu</Link>.</p>
                <p><strong>Plain naan.</strong> The original. Tandoor heat, dough, nothing else. If the base is right, nothing more is needed. At 400 degrees, plain naan earns its place. Serve it with <Link href={`${base}/mutton-rogan-josh-den-haag`} className="text-[#06068a] hover:text-[#0000B3] font-semibold">mutton rogan josh Den Haag</Link> for a combination that speaks for itself.</p>
              </>
            )}
          </div>
        </div>
          ...
          <div className="font-body text-[#1A1A1A]/70 text-lg leading-relaxed space-y-5">
            {isNl ? (
              <>
                ...
                <p>Het timing-raam is smal. Dertig seconden te lang en het droogt uit. Dertig seconden te kort en het midden is rauw. Dit elke avond goed doen, voor elke bestelling, onderscheidt een keuken die jaren heeft gewerkt aan <Link href={`${base}/tandoori-den-haag`} className="text-[#06068a] hover:text-[#0000B3] font-semibold">tandoori koken in Den Haag</Link> van een keuken die gewoon deeg verplaatst. Chopras Indian Restaurant is beoordeeld met 4,9 sterren door 1300+ gasten op Google. Die beoordeling is voor een deel het verhaal van dit brood.</p>
              </>
            ) : (
              <>
      ...
      <section className="btn-gradient  py-20 px-6 md:px-16">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="font-body text-white/80 text-lg leading-relaxed">
            {isNl ? (
              <p className='text-white/80'>Chopras Indian Restaurant op Leyweg 986 in Den Haag bakt vers naan brood in een tandoor kleioven van 400 graden elke avond van dinsdag tot zondag vanaf 16:30. Zes soorten naan zijn beschikbaar: knoflooknaan, boternaan, kaasnaan, Peshwarinaan, keema naan en gewone naan. Het restaurant heeft een 4,9-sterrenbeoordeling op Google van 1300+ gasten en is volledig halal gecertificeerd. <Link href={`${base}/contact`} className="text-white hover:text-white font-semibold">Reserveer een tafel bij Chopras Indian Restaurant</Link> of bestel <Link href={`${base}/indian-takeaway-den-haag`} className="text-white hover:text-white font-semibold">Indiaas takeaway in Den Haag</Link>.</p>
            ) : (
              <p className='text-white/80'>Chopras Indian Restaurant at Leyweg 986 in Den Haag bakes fresh naan bread in a 400-degree tandoor clay oven every evening from Tuesday to Sunday from 16:30. Six types of naan are available: garlic naan, butter naan, cheese naan, Peshwari naan, keema naan, and plain naan. The restaurant holds a 4.9-star rating on Google from 1300+ guests and is fully halal certified. <Link href={`${base}/contact`} className="text-white hover:text-white font-semibold">Reserve a table at Chopras Indian Restaurant</Link> or order <Link href={`${base}/indian-takeaway-den-haag`} className="text-white hover:text-white font-semibold">Indian takeaway in Den Haag</Link>.</p>
            )}
          </div>
        </div>
      </section>
      ...
      <section className="bg-white py-20 px-6 md:px-16">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 text-center">
            <Link href={`${base}/tandoori-den-haag`} className="block p-6 bg-[#F7F8FC] rounded-lg border border-gray-200 hover:border-[#06068a] hover:shadow-lg transition-all">
              <p className="text-[#06068a] text-sm font-semibold uppercase tracking-widest mb-2">{isNl ? 'Tandoor' : 'Tandoor'}</p>
            ...
            <Link href={`${base}/butter-chicken-den-haag`} className="block p-6 bg-[#F7F8FC] rounded-lg border border-gray-200 hover:border-[#06068a] hover:shadow-lg transition-all">
              <p className="text-[#06068a] text-sm font-semibold uppercase tracking-widest mb-2">{isNl ? 'Curry' : 'Curry'}</p>
            ...
            <Link href={`${base}/biryani-den-haag`} className="block p-6 bg-[#F7F8FC] rounded-lg border border-gray-200 hover:border-[#06068a] hover:shadow-lg transition-all">
              <p className="text-[#06068a] text-sm font-semibold uppercase tracking-widest mb-2">{isNl ? 'Rijst' : 'Rice'}</p>
