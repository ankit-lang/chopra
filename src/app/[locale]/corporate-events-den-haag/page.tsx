    ...
    <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-10">
            {eventCards.map((card) => (
              <div key={card.title} className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm text-center">
                ...
                <h3 className="font-heading font-bold text-[#06068a] text-base mb-2">{card.title}</h3>
      ...
      <section className="bg-white py-20 px-6 md:px-16">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="font-body text-[#1A1A1A]/70 text-lg leading-relaxed">
            {isNl ? (
              <p>Ja. Chopras Indian Restaurant op Leyweg 986, Den Haag verzorgt zakelijke evenementen en professionele catering voor groepen van 15 tot 80 gasten. Het restaurant is volledig halal gecertificeerd, met vegetarische en veganistische opties in het <Link href={`${base}/menu`} className="text-[#06068a] hover:text-[#0000B3] font-semibold">volledige menu van 143 gerechten</Link>. Chopras cateert teamdiners, <Link href={`${base}/indian-restaurant-near-peace-palace-den-haag`} className="text-[#06068a] hover:text-[#0000B3] font-semibold">diplomatieke recepties bij het Vredespaleis</Link>, conferentiediners en productlanceringen in de eigen privézaal en op externe locaties door Den Haag. Beoordeeld met 4,9 sterren op Google. Open dinsdag tot en met zondag vanaf 16:30.</p>
            ) : (
              <p>Yes. Chopras Indian Restaurant at Leyweg 986, Den Haag handles corporate events and professional catering for groups of 15 to 80 guests. The restaurant is fully halal certified, with vegetarian and vegan options across its <Link href={`${base}/menu`} className="text-[#06068a] hover:text-[#0000B3] font-semibold">143-dish menu</Link>. Chopras caters team dinners, <Link href={`${base}/indian-restaurant-near-peace-palace-den-haag`} className="text-[#06068a] hover:text-[#0000B3] font-semibold">diplomatic receptions near the Peace Palace</Link>, conference meals and product launches both at its own private hall and at external venues across Den Haag. Rated 4.9 stars on Google from 1300+ reviews. Open Tuesday to Sunday from 16:30.</p>
            )}
          </div>
        </div>
      </section>
      ...
      <section className="bg-white py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="font-heading text-3xl md:text-4xl text-[#06068a] mb-10 text-center">
            {isNl ? 'Andere Cateringmogelijkheden' : 'Other Catering Options'}
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <Link href={`${base}/indian-wedding-catering-den-haag`} className="block p-6 bg-[#F7F8FC] rounded-lg border border-gray-200 hover:border-[#06068a]/30 hover:shadow-lg transition-all">
              <p className="text-[#06068a] text-sm font-semibold uppercase tracking-widest mb-2">{isNl ? 'Bruiloft' : 'Wedding'}</p>
              <p className="text-[#06068a] font-semibold">{isNl ? 'Indiase bruiloftscatering' : 'Indian wedding catering'}</p>
            </Link>
            <Link href={`${base}/feestzaal-den-haag`} className="block p-6 bg-[#F7F8FC] rounded-lg border border-gray-200 hover:border-[#06068a]/30 hover:shadow-lg transition-all">
              <p className="text-[#06068a] text-sm font-semibold uppercase tracking-widest mb-2">{isNl ? 'Feestzaal' : 'Event Space'}</p>
              <p className="text-[#06068a] font-semibold">{isNl ? 'Zakelijk evenement feestzaal Den Haag' : 'Business event space Den Haag'}</p>
            </Link>
            <Link href={`${base}/indian-birthday-catering-den-haag`} className="block p-6 bg-[#F7F8FC] rounded-lg border border-gray-200 hover:border-[#06068a]/30 hover:shadow-lg transition-all">
              <p className="text-[#06068a] text-sm font-semibold uppercase tracking-widest mb-2">{isNl ? 'Verjaardag' : 'Birthday'}</p>
              <p className="text-[#06068a] font-semibold">{isNl ? 'Verjaardagscatering' : 'Birthday catering'}</p>
            </Link>
            <Link href={`${base}/diwali-dinner-den-haag`} className="block p-6 bg-[#F7F8FC] rounded-lg border border-gray-200 hover:border-[#06068a]/30 hover:shadow-lg transition-all">
              <p className="text-[#06068a] text-sm font-semibold uppercase tracking-widest mb-2">{isNl ? 'Diwali' : 'Diwali'}</p>
              <p className="text-[#06068a] font-semibold">{isNl ? 'Diwali-diner' : 'Diwali dinner'}</p>
            </Link>
          </div>
          <div className="text-center space-y-4">
            ...
            <p className="text-[#1A1A1A] text-base">
              ...
              <Link href={`${base}/menu`} className="text-[#06068a] hover:text-[#0000B3] font-semibold">{isNl ? 'cateringsmenu' : 'catering menu'}</Link>
              {' '}{isNl ? 'of' : 'or'}{' '}
              <Link href={`${base}/contact`} className="text-[#06068a] hover:text-[#0000B3] font-semibold">{isNl ? 'maak nu een afspraak' : 'book a consultation now'}</Link>.
            </p>
          </div>
        </div>
      </section>
