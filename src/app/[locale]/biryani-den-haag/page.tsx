      ...
      <section className="relative pt-32 pb-24 overflow-hidden btn-gradient ">
        ...
        <div className="relative max-w-5xl mx-auto px-6 text-center z-10">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link
              href={`${base}/contact`}
              className="inline-flex items-center justify-center gap-2 rounded-full border-2 border-white bg-gradient-to-r from-[#000099] to-[#0000e6] px-8 py-3.5 text-white text-sm font-semibold uppercase tracking-wider transition-all duration-200 ease-out hover:scale-105 active:scale-[0.98] min-h-[48px] shadow-lg"
            >
              {tr.common.reserve}
            </Link>
            <Link
          ...
          <div className="prose prose-lg max-w-none text-[#1A1A1A] space-y-5">
            {isNl ? (
              <>
                ...
                <p>Het resultaat is zichtbaar als het deksel aan tafel afgaat. Sommige korrels zijn goudgeel van de saffraan. Andere zijn wit. Weer andere zijn gekleurd door de <Link href={`${base}/blog/best-indian-restaurant-den-haag`} className="text-[#06068a] font-semibold hover:underline">vers gemalen kruiden</Link>. Die variatie is het bewijs van de methode. Uniforme rijst is het bewijs dat iemand heeft gesneden.</p>
              </>
            ) : (
              <>
                ...
                <p>The result is visible when the lid is lifted at the table. Some grains are golden from the saffron. Others are white. Others are stained by the <Link href={`${base}/blog/best-indian-restaurant-den-haag`} className="text-[#06068a] font-semibold hover:underline">freshly ground spices</Link>. That variation is the proof of the method. Uniform rice is the proof that someone cut corners.</p>
              </>
            )}
          </div>
        </div>
      ...
      <section className="bg-white py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="prose prose-lg max-w-none text-[#1A1A1A] space-y-5">
            {isNl ? (
              <>
                <p>Het begint elke ochtend voor de dienst. De keuken van Chopras Indian Restaurant maalt zijn masalas van hele kruiden die rechtstreeks uit India worden geimporteerd. Komijn, kardemom, korianderzaad, kruidnagel, zwarte peper, kaneelstokjes, laurierblaadjes. De vluchtige aromatische olien in vers gemalen kruiden beginnen te verdampen binnen enkele uren na het malen. Een voorgemengd kruidenmengsel dat weken in een magazijn heeft gelegen, kan daar niet tegenop. Dit is het verschil dat elk gerecht op het <Link href={`${base}/menu`} className="text-[#06068a] font-semibold hover:underline">volledige Chopras Indian Restaurant-menu</Link> kenmerkt.</p>
                <p>De kip wordt de avond ervoor gemarineerd in een yoghurtbasis met verse gember en knoflook. Het lam, lamschouder voor de lambiryani, wordt op dezelfde manier behandeld. Beide zijn afkomstig van <Link href={`${base}/halal-food-den-haag`} className="text-[#06068a] font-semibold hover:underline">volledig halal gecertificeerde leveranciers</Link>. Elke avond, zonder uitzondering. Tegen de tijd dat het vlees de pot bereikt, heeft het al een laag smaak van binnenuit opgenomen.</p>
                <p>De basmatirijst wordt geweekt en gedeeltelijk gekookt, bewust te kort gehouden, omdat hij de rest gaar trekt in de afgesloten pot. Echte saffraandraadjes worden in warm water geweekt en over de bovenste rijstlaag gedruppeld voordat de pot wordt afgesloten. Het deksel gaat op het vuur. De stoom doet de rest. Chopras Indian Restaurant serveert kipbiryani, lambiryani en groentebiryani. Alle drie zijn beschikbaar als onderdeel van het <Link href={`${base}/menu`} className="text-[#06068a] font-semibold hover:underline">volledige Noord-Indiase menu</Link>, beoordeeld met 4,9 sterren op Google op basis van 1100+ recensies.</p>
                <p>Naast biryani serveert Chopras Indian Restaurant ook <Link href={`${base}/tandoori-den-haag`} className="text-[#06068a] font-semibold hover:underline">tandoori-specialiteiten</Link>, <Link href={`${base}/butter-chicken-den-haag`} className="text-[#06068a] font-semibold hover:underline">butter chicken</Link> en <Link href={`${base}/dal-makhani-den-haag`} className="text-[#06068a] font-semibold hover:underline">dal makhani</Link> op Leyweg 986. Voor grote groepen of evenementen is er ook <Link href={`${base}/indian-buffet-den-haag`} className="text-[#06068a] font-semibold hover:underline">Indiaas buffet in Den Haag</Link> beschikbaar.</p>
              </>
            ) : (
              <>
                <p>It starts every morning before service. The kitchen at Chopras Indian Restaurant grinds its masalas from whole spices imported directly from India. Cumin, cardamom, coriander seed, cloves, black pepper, cinnamon sticks, bay leaves. The volatile aromatic oils in freshly ground spices begin to evaporate within hours of grinding. A pre-mixed blend sitting in a warehouse for weeks cannot compete. This is the difference you taste in every dish across the <Link href={`${base}/menu`} className="text-[#06068a] font-semibold hover:underline">full Chopras Indian Restaurant menu</Link>.</p>
                <p>The chicken is marinated overnight in a yoghurt base with fresh ginger and garlic. The lamb, shoulder cut for the lamb biryani, is treated the same way. Both are sourced from <Link href={`${base}/halal-food-den-haag`} className="text-[#06068a] font-semibold hover:underline">fully halal-certified suppliers</Link>. Every evening, without exception. By the time the protein reaches the pot, it has already absorbed a layer of flavour from within the flesh itself.</p>
                <p>The basmati rice is soaked and partially cooked, intentionally left short of done, because it will finish inside the sealed pot. Real saffron threads are steeped in warm water and drizzled across the top layer of rice before the pot is sealed. The lid goes on the heat. The steam does the rest. Chopras Indian Restaurant serves chicken biryani, lamb biryani, and vegetable biryani. All three are available as part of the <Link href={`${base}/menu`} className="text-[#06068a] font-semibold hover:underline">full North Indian menu</Link>, rated 4.9 stars on Google from 1100+ reviews.</p>
                <p>Alongside biryani, Chopras Indian Restaurant also serves <Link href={`${base}/tandoori-den-haag`} className="text-[#06068a] font-semibold hover:underline">tandoori specialities</Link>, <Link href={`${base}/butter-chicken-den-haag`} className="text-[#06068a] font-semibold hover:underline">butter chicken</Link>, and <Link href={`${base}/dal-makhani-den-haag`} className="text-[#06068a] font-semibold hover:underline">dal makhani</Link> at Leyweg 986. For larger groups or events, <Link href={`${base}/indian-buffet-den-haag`} className="text-[#06068a] font-semibold hover:underline">Indian buffet in Den Haag</Link> is also available.</p>
              </>
            )}
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            {[
              ...
              <div key={item.name} className="bg-white rounded-xl p-6 border border-gray-100 shadow-sm">
                <h3 className="font-heading text-xl text-[#06068a] mb-1">{item.name} - {item.price}</h3>
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <Link
              href={`${base}/contact`}
              className="inline-flex items-center justify-center gap-2 rounded-full border-2 border-white  px-6 py-3 text-white text-sm font-medium uppercase tracking-wide transition-all duration-200 ease-out btn-gradient active:scale-[0.98] min-h-[48px]"
            >
              {tr.common.reserve}
            </Link>
            <Link
      ...
      <section className="btn-gradient  py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-white/90 text-lg leading-relaxed">
            {isNl ? (
              <p className='text-white/80'>Authentieke biryani Den Haag vindt u bij Chopras Indian Restaurant, Leyweg 986, 2545 GW Den Haag, open van dinsdag tot en met zondag van 16:30 tot 22:30. Chopras Indian Restaurant serveert kip-, lam- en groentebiryani via de echte dum-methode: afgesloten, langzaam gegaard met echte saffraandraadjes en kruiden die elke ochtend vers worden gemalen van hele ingredienten uit India. Beoordeeld met 4,9 sterren op Google op basis van 1100+ recensies. Volledig <Link href={`${base}/halal-food-den-haag`} className="text-white font-semibold hover:underline">halal gecertificeerd</Link>. <Link href={`${base}/contact`} className="text-white font-semibold hover:underline">Reserveer een tafel</Link> of bestel online via Thuisbezorgd.</p>
            ) : (
              <p className='text-white/80'>Authentic biryani Den Haag is at Chopras Indian Restaurant, Leyweg 986, 2545 GW Den Haag, open Tuesday to Sunday from 16:30 to 22:30. Chopras Indian Restaurant serves chicken, lamb, and vegetable biryani using the authentic dum method: sealed, slow-cooked with real saffron threads and spices ground fresh every morning from whole ingredients sourced in India. Rated 4.9 stars on Google from 1100+ reviews. Fully <Link href={`${base}/halal-food-den-haag`} className="text-white font-semibold hover:underline">halal certified</Link>. <Link href={`${base}/contact`} className="!text-white font-semibold hover:underline">Reserve a table</Link> or order online via Thuisbezorgd.</p>
            )}
          </div>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
            {[
              ...
              <Link
                key={dish.label}
                href={dish.href}
                className="flex items-center justify-center p-5 rounded-2xl bg-gradient-to-r from-[#000099] to-[#0000e6] text-white font-serif text-lg md:text-xl font-medium tracking-wide shadow-md hover:scale-[1.02] transition-transform duration-200 text-center min-h-[64px]"
              >
                {dish.label}
              </Link>
            ))}
          ...
          <div className="mt-8 text-center space-y-4">
            <p className="text-[#1A1A1A] text-base">
              <Link href={`${base}/`} className="text-[#06068a] font-semibold hover:underline">
